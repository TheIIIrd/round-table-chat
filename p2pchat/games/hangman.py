...
class Hangman:
    ...
    min_players = 1
    max_players = 8
    verbs = frozenset({"буква", "слово", "виселица"})
    ...
    def start(self, players: Sequence[str]) -> list[Action]:
        ...
        return [
            Say(
                ...
                "!буква <а> — назвать букву, !слово <ответ> — назвать слово целиком\n"
                + self.render()
            )
        ]
    ...
    def handle(self, player: str, verb: str, rest: str) -> list[Action]:
        ...
        self._arm(time.monotonic())
        return self._word(player, rest) if verb == "слово" else self._letter(player, rest)
    ...
    def render(self) -> str:
        ...
        return f"{self.masked()}\nМимо: {wrong}   Осталось попыток: {left}  {gallows}"
