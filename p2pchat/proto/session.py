...
# Версия протокола. Поднимается при ЛЮБОМ изменении формата на проводе —
# кадров, конвертов, порядка сообщений хендшейка.
#
# Версия входит в prologue, то есть в хеш стенограммы: несовпадение ломает
# хендшейк громко и сразу. Без подъёма версии два клиента с разным форматом
# успешно соединятся и начнут читать друг друга со сдвигом — файл придёт битым,
# текст превратится в мусор, а искать причину придётся часами.
#
# 2: из конверта убраны часы Лампорта, заголовок сократился с 17 байт до 9.
PROTOCOL_VERSION = b"p2pchat/2"
